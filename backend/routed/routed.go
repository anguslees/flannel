...
func (rb *RoutedBackend) Init(extIface *net.Interface, extIP net.IP, ipMasq bool) (*backend.SubnetDef, error) {
	...
	/* Let docker set up the local route
	localRoute := netlink.Route{
		Scope: netlink.SCOPE_LINK,
		Dst:   sn.ToIPNet(),
	}
	if err := netlink.RouteAdd(&localRoute); err != nil {
		return nil, fmt.Errorf("Failed to add local route to %v: %v", sn, err)
	}
	*/
...
func (rb *RoutedBackend) Stop() {
	close(rb.stop)
}

func (rb *RoutedBackend) Name() string {
	return "routed"
}

func (rb *RoutedBackend) handleSubnetEvents(batch subnet.EventBatch) {
	for _, evt := range batch {
		switch evt.Type {
		case subnet.SubnetAdded:
			log.Infof("Subnet added: %v via %v", evt.Lease.Network, evt.Lease.Attrs.PublicIP)

			if evt.Lease.Attrs.BackendType != "routed" {
				log.Warningf("Ignoring non-routed subnet: type=%v", evt.Lease.Attrs.BackendType)
				continue
			}
